...
function getEmbedUrl(url) {
    ...
    if (url.includes('rutube.ru')) {
        ...
        if (videoId) {
            return `https://rutube.ru/embed/${videoId}/?autoplay=0`;
        }
        return url;
    }
...
window.showMatchesView = function() {
    ...
    html += `</div>`;
    if (upcoming.length === 0 && liveMatches.length === 0) html += `<div style="text-align:center; padding: 40px;">Нет предстоящих событий. Добавьте первый матч в админ-панели!</div>`;
        ...
        appDiv.innerHTML = `
            ...
            <div class="player-container" style="background: #0a0a0f; text-align: center; padding: 40px 20px;">
                ...
                <div style="margin: 40px 0;">
                    <i class="fas fa-external-link-alt" style="font-size: 64px; color: #ff3b00; opacity: 0.8;"></i>
                    <h3 style="margin: 20px 0 10px;">Трансляция на Match TV</h3>
                    <p style="color: #aaa; margin-bottom: 30px;">Нажмите на кнопку ниже, чтобы открыть матч на официальном сайте</p>
                    <button onclick="openDirect('${escapeHtml(directUrl)}')" class="stream-btn" style="max-width: 300px; margin: 0 auto;">
                        <i class="fas fa-play"></i> Смотреть на Match TV
                    </button>
                </div>
                <div style="background: rgba(255,59,0,0.1); border-radius: 20px; padding: 16px; margin-top: 20px;">
                    <i class="fas fa-info-circle"></i> 
                    Видео открывается в новой вкладке — так работает официальный сайт Match TV.
                </div>
            </div>
        `;
        return;
    ...
    appDiv.innerHTML = `
        ...
        <div class="player-container">
            ...
            <div style="background:#0a0a0f; padding:14px; font-size:14px; color:#aaa; text-align:center; border-top:1px solid #222;">
                <i class="fas fa-info-circle"></i> Если видео не работает — нажмите кнопку <strong>"Открыть в новой вкладке"</strong>.
            </div>
        </div>
    `;
};
...
window.editMatch = function(id) {
    ...
    const newUrl = prompt('Ссылка на трансляцию (Rutube, YouTube, Twitch...)', match.streamUrl);
