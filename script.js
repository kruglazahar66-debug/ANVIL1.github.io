...
function initData() {
    ...
    autoUpdateInterval = setInterval(() => {
        if (currentUser && currentUser.role === 'admin') {
            aiManageSchedule();
            if (document.getElementById('app')) showMatchesView();
        } else {
            matches.forEach(m => m.status = getMatchStatus(m));
            saveMatches();
            if (document.getElementById('app')) showMatchesView();
...
window.showAdminPanel = function() {
    ...
    appDiv.innerHTML = `<div class="admin-panel">
        ...
        <div class="admin-form">
            ...
            <button class="admin-btn" onclick="aiManageSchedule(); saveMatches(); showMatchesView();" style="background:#0d6efd; margin-bottom:20px;"><i class="fas fa-sync-alt"></i> Запустить AI сейчас</button>
