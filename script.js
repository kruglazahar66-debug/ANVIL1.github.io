...
function getEmbedUrl(url) {
    ...
    if (url.includes('rutube.ru')) {
        ...
        if (url.includes('/video/')) {
            ...
            videoId = url.split('/play/embed/')[1]?.split('/')[0]?.split('?')[0];
        } else if (url.includes('/embed/video/')) {
            videoId = url.split('/embed/video/')[1]?.split('/')[0]?.split('?')[0];
        }
        if (videoId) {
            return `https://rutube.ru/play/embed/${videoId}?autoplay=0`;
        }
    }
    return url;
}
...
function getDirectUrl(url) {
    ...
    if (url.includes('rutube.ru')) {
        if (url.includes('/play/embed/')) {
            let videoId = url.split('/play/embed/')[1]?.split('?')[0];
            if (videoId) return `https://rutube.ru/video/${videoId}/`;
        }
        if (url.includes('/embed/video/')) {
            let videoId = url.split('/embed/video/')[1]?.split('?')[0];
            if (videoId) return `https://rutube.ru/video/${videoId}/`;
        }
        return url;
    }
...
window.editMatch = function(id) {
    ...
    const newUrl = prompt('Ссылка на трансляцию (Match TV, YouTube, Twitch, Rutube...)', match.streamUrl);
...
(function() {
    ...
    mobileStyles.textContent = `
        @media (max-width: 768px) {
            body { font-size: 14px; }
            .nav-links { flex-wrap: wrap; gap: 8px; padding: 12px 10px; }
            .nav-btn, .logout-btn { padding: 10px 14px; font-size: 13px; flex: 1 0 auto; min-width: 70px; }
            .user-info { font-size: 13px; padding: 6px 12px; }
            .hero h1 { font-size: 28px; }
            .hero p { font-size: 14px; }
            .matches-grid { grid-template-columns: 1fr !important; gap: 15px; padding: 15px; }
            .match-card { padding: 16px; margin: 0; }
            .match-title { font-size: 16px; }
            .match-datetime, .match-preview { font-size: 12px; }
            .stream-btn, .player-action-btn, .admin-btn, .edit-btn, .delete-btn { padding: 12px 16px; font-size: 14px; min-height: 48px; width: 100%; margin-top: 10px; }
            .player-container { padding: 0; border-radius: 0; }
            .player-frame { width: 100%; height: 45vh; }
            .player-header { flex-direction: column; align-items: flex-start; gap: 10px; padding: 12px; }
            .player-actions { flex-wrap: wrap; gap: 8px; width: 100%; }
            .player-action-btn { flex: 1; text-align: center; padding: 10px; font-size: 12px; }
            .admin-form input, .admin-form .form-group { width: 100%; }
            .admin-panel { padding: 15px; }
            .match-item { flex-direction: column; align-items: flex-start; gap: 10px; padding: 12px; }
            .match-info { width: 100%; }
            .live-banner { flex-direction: column; margin: 15px; }
            .auth-card { width: 90%; margin: 40px auto; padding: 25px 20px; }
            .auth-input, .auth-btn { font-size: 16px; padding: 14px; }
            .badge { font-size: 10px; padding: 4px 8px; }
        }
        .match-card, .nav-btn, .logout-btn, .player-action-btn, .stream-btn, .edit-btn, .delete-btn {
            cursor: pointer;
            touch-action: manipulation;
        }
        .player-frame { border: none; background: #000; }
        @keyframes pulse {
            0% { transform: scale(1); background-color: #ff5e2e; }
            50% { transform: scale(1.05); background-color: #ff7b4a; }
            100% { transform: scale(1); background-color: #ff5e2e; }
        }
    `;
    ...
    window.toggleFullscreen = function() {
        ...
        if (frame) {
            ...
            else alert("Нажмите 'Открыть в новой вкладке' для просмотра в полном размере");
        }
    };
    ...
    window.addEventListener('resize', function() {
        ...
        if (frame && window.innerWidth <= 768) {
            frame.style.height = window.innerHeight > window.innerWidth ? '40vh' : '70vh';
        }
    });

    const originalWatchStream = window.watchStream;
    window.watchStream = function(matchId) {
        originalWatchStream(matchId);
        if (window.innerWidth <= 768) {
            setTimeout(() => {
                const fullscreenBtn = document.querySelector('.player-action-btn:last-child');
                if (fullscreenBtn && fullscreenBtn.innerText.includes('На весь экран')) {
                    fullscreenBtn.style.animation = 'pulse 0.6s 2';
                }
            }, 500);
        }
    };
