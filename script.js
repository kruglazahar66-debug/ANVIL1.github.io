...
function initData() {
    ...
    if (!storedMatches) {
        const defaultMatches = [
            { id: 'm1', title: '🎬 YouTube: Трейлеры и трансляции', datetime: getFutureDate(0), streamUrl: 'https://www.youtube.com/watch?v=dQw4w9WgXcQ', status: 'upcoming' },
            { id: 'm2', title: '🎮 Twitch: Игровые стримы', datetime: getFutureDate(1), streamUrl: 'https://www.twitch.tv/riotgames', status: 'upcoming' },
            { id: 'm3', title: '🏆 Vimeo: Спортивные события', datetime: getFutureDate(2), streamUrl: 'https://vimeo.com/76979871', status: 'upcoming' }
        ];
...
window.watchStream = function(matchId) {
    ...
    const originalUrl = match.streamUrl;
    ...
    app.innerHTML = `
        ...
        <div class="match-card">
            ...
            <div style="margin-bottom: 15px; display: flex; gap: 10px; flex-wrap: wrap;">
                <button onclick="window.openInNewTab('${escapeHtml(originalUrl)}')" class="stream-link" style="background: #28a745;">🔗 Открыть в новой вкладке</button>
                <button onclick="window.toggleFullscreen()" class="stream-link" style="background: #17a2b8;">🖥 Полный экран</button>
            </div>
            <div class="iframe-container" id="playerContainer">
                <iframe id="streamFrame" src="${escapeHtml(embedUrl)}" allowfullscreen allow="autoplay; encrypted-media; fullscreen" sandbox="allow-same-origin allow-scripts allow-popups allow-forms allow-presentation allow-popups-to-escape-sandbox"></iframe>
            </div>
            <div style="margin-top: 15px; padding: 12px; background: #1e2538; border-radius: 8px; font-size: 13px; color: #ffaa88;">
                💡 Если видео не загружается — нажмите "Открыть в новой вкладке"
            </div>
        </div>
    `;
};
    ...
    app.innerHTML = `
        <div class="admin-panel">
            ...
            <div class="form-group">
                <label>🎬 Как вставлять ссылки (работает без ошибок):</label>
                <div style="background:#1e2538; padding:12px; border-radius:8px; font-size:12px; margin-bottom:15px;">
                    <strong>YouTube:</strong> https://www.youtube.com/watch?v=ID_ВИДЕО<br>
                    <strong>YouTube (короткая):</strong> https://youtu.be/ID_ВИДЕО<br>
                    <strong>Twitch (канал):</strong> https://www.twitch.tv/ИМЯ_КАНАЛА<br>
                    <strong>Vimeo:</strong> https://vimeo.com/ID_ВИДЕО
                </div>
            </div>
            ...
            <div class="form-group">
                <label>URL трансляции</label>
                <input type="text" id="newUrl" placeholder="https://www.youtube.com/watch?v=... или https://www.twitch.tv/...">
            </div>
